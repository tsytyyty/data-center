<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column height="1000" label="提单号" prop="id" sortable="custom" align="center" width="190" :class-name="getSortClass('id')">
      ...
      <el-table-column label="货主代码" prop="id" sortable="custom" align="center" width="130" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.ownerId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="物流公司" prop="id" sortable="custom" align="center" width="160" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.companyId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="集装箱箱号" prop="id" sortable="custom" align="center" width="130" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.containerId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="货物名称" prop="id" sortable="custom" align="center" width="130" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
...
/*    console.log(this.list1)
  ...
  methods: {
    getTotal(){
      axios.get(`http://59.110.34.218:10010/analysis-server/error/data/errorTotal?type=logisticsError`).then(
        response => {
    ...
    getList() {
      ...
      axios.get(`http://59.110.34.218:10010/analysis-server/error/data/list/logisticsError?page=${this.currentPage}&number=20`).then(
        response => {
          console.log(response.data.object)
          if(response.data.code == 0) {
